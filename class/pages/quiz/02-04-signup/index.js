...
import {
  ...
  GenderRadioWoman,
  GenderRadioMan,
} from "../02-04-signup/02-04-signup-emotion";
...
const QuizSignUpPage=()=> {
  ...
  function onChangeEmail(event) {
    ...
    if (event.target.value === "") {
      setEmailError("이메일 쓰세요");
    }if (event.target.value.includes("@") === false){
        setEmailError("형식이 안맞잖슴")
    }else {
        setEmailError("")
    }
  ...
  function onchangePhone2(event) {
    ...
    // if(phone2.length === 4){ useRef 검색해봐
    //     phone3.focus()
    // }
  }
  ...
  return (
    <Wrapper>
      ...
      <TokenWrapper>
        <Token>{number}</Token>
        <TokenButton onClick={handleNumber} disabled>
          인증번호전송
        </TokenButton>
      </TokenWrapper>
      <TokenWrapper>
      ...
      <GenderWrapper>
        <GenderRadioWoman type="radio" name="gender" />
        여성
        <GenderRadioMan type="radio" name="gender" />
        남성
      </GenderWrapper>
